# Spec: ElasticsearchEngine

## Purpose

The `ElasticsearchEngine` implements the `Engine` interface against an Elasticsearch cluster using the ES|QL query language. It provides index discovery, field schema introspection, sample document retrieval, and query execution, and manages the lifecycle of the underlying `AsyncElasticsearch` connection pool.
