...
## Purpose
How Benny loads and resolves runtime configuration: TOML-canonical settings via `pydantic-settings`, env-over-TOML-over-defaults precedence, env-only secrets, and the committed `config.toml.example` / git-ignored `config.toml` convention.
