...
* **Human Access Layer:** Exposes the agent to human analysts via their personal GenAI CLIs (Claude Code, Gemini CLI) — see Section 8.
...
## 6. Investigation State & Deduplication

Once an investigation is complete, the full conclusion (Incident Report) is persisted to a dedicated results store. This serves two purposes:

* **State:** A permanent record of every investigated alert — verdict, findings, recommended actions, and the runbook used.
* **Deduplication / Fast Lookup:** Before starting a new investigation, the system checks whether an identical or equivalent alert has already been analyzed. If a match is found, the stored result is returned immediately without re-running the full agent pipeline.

This is particularly valuable for high-volume environments where the same alert type may fire repeatedly (e.g., repeated brute-force attempts from the same IP). The deduplication logic is based on alert fingerprinting (e.g., alert type + key indicators).

---

## 7. Testing & Evaluation (LLM-as-a-Judge)
...
## 8. Human Access via GenAI CLIs

In addition to automated SIEM ingestion, the agent supports two distinct human access patterns with different requirements.

### Pattern 1: Structured Alert Submission — REST API + Skill

For submitting known alert types and retrieving results, the agent exposes a **REST API with an OpenAPI specification**. A corresponding **Agent Skill definition** is published in this repository. Analysts register it once; the CLI reads the spec and knows how to call the API.

* Works for well-defined, structured investigations (same as SIEM ingestion).
* Skill updates ship with the API as part of the normal repo-driven build cycle.

### Pattern 2: Ad-Hoc Interactive Investigation — MCP Server

For open-ended, conversational investigations — e.g., *"I noticed something odd in the GitHub audit logs, take a look and tell me if anything is suspicious"* — the agent exposes an **MCP Server**.

This use case requires a **persistent session**: the analyst may ask follow-up questions, redirect the investigation, or narrow scope across multiple turns. A REST Skill cannot maintain this context across messages — every call is stateless and loses prior findings.

MCP's persistent session makes this natural:
* Context accumulates across turns within a session.
* The analyst can say "dig deeper into user X" without re-explaining the full context.
* Supports arbitrary, unstructured investigations beyond predefined Runbooks.

### Exposed Endpoints (REST)

* **`POST /investigate`** — Submit a structured alert for investigation, returns an Incident Report.
* **`GET /investigations/{id}`** — Retrieve a past investigation result.
* **`GET /runbooks`** — List available Runbooks and their descriptions.

### Access Model

Both interfaces are read/trigger-only — analysts can submit alerts and retrieve results, but cannot modify runbooks, skills, or configuration. All configuration changes go through the repo and trigger a new build.
